...
export class LineService {
	...
	update(line: ILine): Observable<EntityResponseType> {
		return this.http.put<ILine>(`${this.resourceUrl}/${getLineIdentifier(line) as number}`, line, {
			observe: 'response',
		});
	}

	partialUpdate(line: ILine): Observable<EntityResponseType> {
		return this.http.patch<ILine>(`${this.resourceUrl}/${getLineIdentifier(line) as number}`, line, {
			observe: 'response',
		});
	}
	...
	getAllRevisions(): Observable<string[]> {
		return this.http.get<string[]>(this.applicationConfigService.getEndpointFor('api/lines/revisions'));
	}

	getRevisionsByProject(projectId: number): Observable<string[]> {
		return this.http.get<string[]>(
			`${this.applicationConfigService.getEndpointFor('api/projects')}/${projectId}/revisions`
		);
	}

	getRevisionsByBlock(blockId: number): Observable<string[]> {
		return this.http.get<string[]>(
			`${this.applicationConfigService.getEndpointFor('api/blocks')}/${blockId}/revisions`
		);
	}
}
