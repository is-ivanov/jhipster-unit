...
export class ProjectService {
	...
	update(project: IProject): Observable<EntityResponseType> {
		return this.http.put<IProject>(`${this.resourceUrl}/${getProjectIdentifier(project) as number}`, project, {
			observe: 'response',
		});
	}

	partialUpdate(project: IProject): Observable<EntityResponseType> {
		return this.http.patch<IProject>(`${this.resourceUrl}/${getProjectIdentifier(project) as number}`, project, {
			observe: 'response',
		});
	}
	...
	addProjectToCollectionIfMissing(
		projectCollection: IProject[],
		...projectsToCheck: (IProject | null | undefined)[]
	): IProject[] {
